			...
			if (SUCCEEDED(hr))
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					if (SUCCEEDED(hr))
					{
						...
						if (!wFilePath.empty()) {
							int size_needed = WideCharToMultiByte(CP_UTF8, 0, &wFilePath[0], (int)wFilePath.size(), NULL, 0, NULL, NULL);
							filePath = std::string(size_needed, 0);
							WideCharToMultiByte(CP_UTF8, 0, &wFilePath[0], (int)wFilePath.size(), &filePath[0], size_needed, NULL, NULL);
